...
(function() {
  ...
  const history = [];
  const maxHistory = 8;
  const MODEL_STORAGE_KEY = 'ai_chat_model_id';
  ...
  function init() {
    ...
    if (modelSelect) {
      modelSelect.addEventListener('change', function() {
        ...
        saveModelPreference(selectedModelId);
      });
    }
  ...
  function setSending(state) {
    isSending = state;
    var chatDisabled = state || !hasAvailableModel;
    if (sendBtn) {
      sendBtn.disabled = chatDisabled;
    }
    if (inputEl) {
      inputEl.disabled = chatDisabled;
    }
    if (modelSelect) {
      modelSelect.disabled = state || !hasAvailableModel;
    }
    if (deepSearchModeSelect) {
  ...
  function renderModelOptions(models, defaultModelId) {
    ...
    models.forEach(function(model) {
      ...
      if (model.available) availableIds.push(model.id);
    });
    ...
    if (!hasAvailableModel) {
      ...
      emptyOption.value = '';
      emptyOption.textContent = 'Sin modelos configurados';
      modelSelect.appendChild(emptyOption);
      selectedModelId = null;
      if (inputEl) inputEl.placeholder = 'Configura una API key para usar el chat...';
      setSending(false);
      return;
    }

    var stored = loadModelPreference();
    if (availableIds.indexOf(stored) >= 0) {
      selectedModelId = stored;
    } else if (availableIds.indexOf(defaultModelId) >= 0) {
    ...
    saveModelPreference(selectedModelId);
    if (inputEl) inputEl.placeholder = 'Escribe tu mensaje...';
    setSending(false);
  }
  ...
  async function loadModelCatalog() {
    if (!modelSelect) return;
    try {
      var response = await fetch('/api/ai/models');
      var data = await response.json();
      if (!response.ok) throw new Error(data.error || 'No se pudo cargar el catalogo');
      renderModelOptions(Array.isArray(data.models) ? data.models : [], data.default_model_id);
    } catch (_err) {
