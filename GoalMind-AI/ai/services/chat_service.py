...
def run_chat(message, history, deep_search_mode=None, model_id=None):
...
def stream_chat(message, history, deep_search_mode=None, model_id=None):
    """Versión streaming de run_chat. Genera eventos (type, data) para SSE."""
    user_message = (message or "").strip()
    if not user_message:
        raise ValueError("Mensaje vacio")
