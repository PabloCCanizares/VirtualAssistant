...
def _build_update_fields(params: Dict[str, Any], allowed_keys: set) -> Dict[str, Any]:
    """Extract only allowed update fields from action parameters."""
    updates = {}
...
def _result(message: str, result_id: Optional[str] = None) -> Dict[str, Any]:
    """Build the common action result payload used by graph and queue execution."""
    return {
...
def execute_action(state: AppState) -> AppState:
    """Execute one planned app action.

    This is intentionally framework-agnostic: graph nodes, future agent runtimes,
    and HTTP use cases can call the same action contract.
    """
    ...
    try:
        ...
        logger.exception("execute_action: fallo al ejecutar accion '%s'", action_name)
